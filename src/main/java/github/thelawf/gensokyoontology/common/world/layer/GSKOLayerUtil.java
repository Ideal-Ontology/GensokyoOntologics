...
public class GSKOLayerUtil extends Layer {
    ...
    public static <T extends IArea, C extends IExtendedNoiseRandom<T>> IAreaFactory<T> buildBiomes(final LongFunction<C> context, Registry<Biome> registry) {
        ...
        area = UntroddenValleyLayer.INSTANCE.setup(registry).apply(context.apply(1006L), area);
        area = NamelessHillLayer.INSTANCE.setup(registry).apply(context.apply(1007L), area);
        area = MistyBankLayer.INSTANCE.setup(registry).apply(context.apply(1008L), area);
