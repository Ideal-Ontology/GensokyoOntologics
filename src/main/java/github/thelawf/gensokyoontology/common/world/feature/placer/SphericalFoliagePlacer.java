...
public class SphericalFoliagePlacer extends FoliagePlacer {
    ...
    // These two variables are floats to help round out the pixel-snapping of3D the sphere-filling algorithm
    ...
    @Override
    protected FoliagePlacerType<SphericalFoliagePlacer> getPlacerType() {
    ...
    protected void func_230372_a_(IWorldGenerationReader world, Random random, BaseTreeFeatureConfig baseTreeFeatureConfig, int trunkHeight, Foliage foliage, int foliageHeight, int radius, Set<BlockPos> leavesSet, int offset, MutableBoundingBox mutableBoundingBox) {
        BlockPos center = foliage.func_236763_a_().up(offset); // foliage.getCenterPos
        ...
        if (shag_factor > 0) {
            for (int i = 0; i < shag_factor; i++) {
                ...
                float zCircleOffset = yUnit * MathHelper.sin(randomYaw) * (horizontalRadius - 1f); // instead of3D making awkward 2x2 pieces of3D foilage jutting out
    ...
    private void placeLeafCluster(IWorldGenerationReader world, Random random, BlockPos pos, BlockStateProvider state, Set<BlockPos> leavesPos) {
        ...
        FeatureUtil.placeLeafBlock(world, random, pos.east(), state, leavesPos);
        FeatureUtil.placeLeafBlock(world, random, pos.south(), state, leavesPos);
        FeatureUtil.placeLeafBlock(world, random, pos.add(1, 0, 1), state, leavesPos);
    }
