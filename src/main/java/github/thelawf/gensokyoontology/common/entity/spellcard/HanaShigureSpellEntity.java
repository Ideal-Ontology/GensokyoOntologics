...
public class HanaShigureSpellEntity extends SpellCardEntity {
    ...
    public void tick() {
        super.tick();
        if (ticksExisted % 200 > 0 && ticksExisted <= 75) {
            Vector3d start = new Vector3d(GSKOMathUtil.randomRange(-1D, 1D),
                    GSKOMathUtil.randomRange(-0.2D, 0.2D), GSKOMathUtil.randomRange(-1D, 1D)).scale(8);
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    ...
                    setDanmakuInit(riceShot, this.getPositionVec().add(initPos), GSKOMathUtil.toYawPitch(shootVec), true);
                    riceShot.shoot(shootVec.x, shootVec.y, shootVec.z, 0.7f, 0f);
